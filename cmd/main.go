...
func main() {
	...
	confluenceProbe, err := probe.NewProbe(probeConfig)
	if err != nil {
	...
	if err := confluenceProbe.Start(); err != nil {
		log.Fatalf("start probe: %v", err)
	}

	log.Println("Confluence probe running; Ctrl+C to stop")

	<-sigChan
	confluenceProbe.Stop()
	log.Println("Confluence probe stopped")
}
